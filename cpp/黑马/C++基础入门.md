...
![image-20220425202442633](D:\MarkDown\picture\image-20220425202442633.png)
...
| **数据类型**        | **占用空间**                                    | 取值范围         |
| ------------------- | ----------------------------------------------- | ---------------- |
| short(短整型)       | 2字节，一个字节8位                              | (-2^15 ~ 2^15-1) |
...
可以发现，ASCII码一共128个，因此在某些力扣题中可以利用这个128来储存字符
...
1. **C++风格字符串**：  `string  变量名 = "字符串值"`
   ...
   int main() {
   	...
   	string s;
       cin>>s;  //输入字符串Hello, whirled!
       cout<<s<<endl;  //输出字符串 Hello,
   	system("pause");
...
> 注意：string定义于头文件#include\<string>
...
![数字越大，优先级越高](D:\MarkDown\picture\image-20220415234140006.png)
...
前置是在语句运算前就进行相应操作，如b=++a是先a自增后赋值给b
后置是语句先运算，后进行相应增或减操作，如b=a++是先给b赋值，后a++，a的自增与b的运算没有关系
...
* 顺序结构：程序按顺序执行，不发生跳转
* 选择结构：依据条件是否满足，有选择的执行相应功能(if，switch，三目)
* 循环结构：依据条件是否满足，循环多次执行某段代码(while，for)
...

{
	...
	case 结果1：执行语句;break;//没有break的话程序会一直向下执行
...
#include<iostream>
#include<string>//string定义字符串必须引用
#include<ctime>//time系统时间
#include <cstdlib> //没有这个会出现system不明确？？？？？？？？？？？
using namespace std;

int main() {

	//添加随机数种子，利用当前系统时间生成随机数，防止每次生成随机数都一样（不加的话，比如这次会每次运行随机数都为42）
	...
	while (num != sj)
	{
		cin >> num;
		if (num > sj) {
			cout << num<<"过大，请输入新数字" << endl;

		}
		else if(num < sj) {
			cout <<num<< "过小，请输入新数字" << endl;
		}
		else {
			cout << "对了" << endl;
			break;
...
**注意：**与while的区别在于==do...while会**先执行一次循环语句**==，再判断循环条件
...
![img](D:\MarkDown\picture\OD5BX}531ILP]5M`H$WDG.jpg)
...
> 总结：**所有指针类型在32位操作系统下是4个字节**，在64位系统下是8字节
...
```c++
////////////////// const int *p 指针常量 ////////////////////
////////修饰的是指针， 指针指向可以改，指向的值不能改////////
/////////////////const 修饰的是*p，因此*p不能改//////////////

////////////////// int* const p 常量指针 ////////////////////
///////修饰的是常量， 指针指向不可以改，指向的值可以改///////
//////////////////const 修饰的是p，因此p不能改///////////////
```
...
int main() {
	...
	//const修饰的是*p1，指针指向可以改，比如从指向a改为指向b
    //指针指向的值不可以更改，比如int a=10;const int* p=&a;这时如果*p=20；就是错误的，无法改变&a这个地址里的数值
	const int * p1 = &a; 
	p1 = &b; //正确
	//*p1 = 100;  报错
...
> 技巧：看const右侧紧跟着的是指针p还是常量*p, 是指针p就是常量指针，是常量 *p就是指针常量
...
```c++
//冒泡排序函数
void bubbleSort(int * arr, int len)  //int * arr 也可以写为int arr[]，因为数组名可以当作指向该数组第一个元素的指针，	且在形参中不需要指定数组的大小，传入一个指针就行，想要确定几行几列的话还需要另外定义参数进行传入
{
...
[c++ 此声明没有存储类或类型说明符](https://blog.csdn.net/weixin_44997676/article/details/106602594)
...
* 值传递（值传递过多会占用很多内存）
* 地址传递（形参改为指针，可以减少内存空间，而且不会复制新的副本出来）
...
```c++
//在地址传递中，通过stu->age = 28;修改了实参的值，这个参数因此改变了
//而在值传递中，仅对形参进行了修改，只体现在子函数里，主函数中并无改变
void printStudent2(student *stu)
{
	stu->age = 28;
	cout << "子函数中 姓名：" << stu->name << " 年龄： " << stu->age  << " 分数：" << stu->score << endl;
...
void fuzhi(teacher* p) {
	for (int i = 0; i < 3; i++) {
		...
		p->name = tname + seed[i];
		for (int j = 0; j < 5; j++) {
			p->stu[j].name = sname + seed[j];
			p->stu[j].score = rand() % 100 + 1;
		}
		p++;
	}
}
...
void fuzhi(teacher tea[]) {
	for (int i = 0; i < 3; i++) {
		string tname = "teacher";
		string sname = "student";
		string seed = "ABCDE";
		tea[i].name = tname + seed[i];
		for (int j = 0; j < 5; j++) {
			tea[i].stu[j].name = sname + seed[j];
			tea[i].stu[j].score = rand() % 100 + 1;
		}
	}
}
