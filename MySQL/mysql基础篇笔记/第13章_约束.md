...
- 非空约束只能出现在表对象的列上，只能某个列单独限定非空，不能组合多个列设置非空。即只有表级约束
...
insert into student values(3,'李四');#失败，后面几个没给值，就成NULL了
...
> 注意，当字段名这列有一条为 NULL ，我们就无法ALTER。只有要修改约束的这个字段中所有数据都不为NULL才能修改约束
...
create table 表名称(
    ...
    字段名  数据类型,  
    字段名  数据类型,
    [constraint 约束名] unique key(字段名)  # 在所有字段的后面声明的约束，为表级约束
);
```
...
alter table 表名称 
add constraint 名字 unique key(字段列表); 
```

```mysql
#方式2：
alter table 表名称 
modify 字段列表 字段类型 unique;
```
...
create table 表名称(
    ...
    字段名  数据类型,  
    字段名  数据类型,
    [constraint 名字] unique key(字段列表) #字段列表中写的是多个字段名，多个字段名用逗号分隔，表示那么是复合唯一，即多个字段的组合是唯一的
);
```
...
(2, 1, 1002, 90),
(3, 2, 1001, 88),
(4, 2, 1002, 56); # 成功，只有两个字段都相同才是唯一，有一个不同就可以
```
...
- 添加唯一性约束的列上也会自动创建**唯一索引**。
- **删除唯一约束只能通过删除唯一索引的方式删除**。
- 删除时需要指定唯一索引名，**唯一索引名就和唯一约束名一样**。
- 如果创建唯一约束时未指定名称，如果是单列，就默认和**列名**相同；如果是组合列，那么默认和()中排在第一个的列名相同。也可以自定义唯一性约束名。
...
- 主键约束相当于**唯一约束 + 非空约束的组合**，若是表级约束，主键约束的组合不允许重复，且每个元素都不能出现空值。如果仅是 UNIQUE，还是会出现多个NULL的情况
...
- 如果是多列组合的复合主键约束，那么**这些列都不允许为空值NULL**，并且**组合的值不允许重复**。
- **MySQL中的主键名总是PRIMARY**，就算自己命名了主键约束名也没用。
...
create table 表名称(
    ...
    字段名  数据类型,  
    字段名  数据类型,
    primary key(字段名) # 表级模式，这里CONSTRAINT起名是没必要的，主键名总是PRIMARY
);
```
...
insert into temp values(null,'李琦'); # 失败，非空
...
```mysql
CREATE TABLE emp4(
id INT PRIMARY KEY AUTO_INCREMENT , # AUTO_INCREMENT用于主键自增长
NAME VARCHAR(20)
);
```
...
insert into student_course values(1, 1001, 89),(1,1002,90),(2,1001,88),(2,1002,56); # 可以
```

```mysql
insert into student_course values(1, 1001, 100); # 报错
...
### 4.6 删除主键约束（在实际开发中不会去删除主键约束）

> 添加主键约束后，会自动的在这个字段上增加主键索引，而我们的B+树就是根据这个主键索引构建的，要是删除，B+树的特征就都删掉了
...
一般是作用在主键上，作用是让某个字段的值自增

比如在一个表中有int型的一个数据，并声明了主键约束。在我们加入数据的时候，本来是需要将这个值加入为1、2、3等，增加了auto_increment后，就可以实现自增（若是这个字段中没有数据的话是从1开始自增，若有数据的话是从上一个数据开始自增），在INSERT增加数据的时候，就可以不写入这个字段，这个字段会自增。
...
（3）**自增长列约束的列必须是键列**（通常结合主键列、唯一键列使用）

（4）自增约束的列的数据类型必须是**整数类型**

（5）如果自增列指定了 0 和 null，会在当前最大值的基础上自增；如果自增列手动指定了具体值，直接赋值为具体值。`但在实际开发中，一旦主键作用的字段上声明有AUTO_INCREMENT，则我们在添加数据时，就不要给主键对应的字段去赋值了`

错误演示：

```mysql
create table employee(
	eid int auto_increment, # 没有加主键约束
    ename varchar(20)
);
...
create table employee(
	eid int primary key,
    ename varchar(20) unique key auto_increment # 类型不满足，自增只能用于整型
);
...
> * 而在MySQL 8.0版本，就实现了自增变量的持久化，自增主键的计数器持久化到`重做日志`中。每次计数器发生改变，都会将其写入重做日志中。
...
**重启数据库**(管理员身份运行net stop mysql80、net start mysql80)，重新插入一个空值。
...
从结果可以看出，新插入的0值分配的是4，按照重启前的操作逻辑，此处应该**分配6**。出现上述结果的主要原因是自增主键`没有持久化`。
这是因为在MySQL 5.7系统中，对于自增主键的分配规则，是由InnoDB数据字典内部一个`计数器`来决定的，而该计数器只在`内存中维护`，并不会持久化到磁盘中。当数据库重启时，该计数器会被初始化。
...
限定某个表的某个字段的引用完整性。(`引用完整性（Referential Integrity）`：例如：员工所在部门，在部门表中要能找到这个部门)

比如：员工表的员工所在部门的选择，必须在部门表能找到对应的部分。在这个图中，上面的表为主表/父表，下面的表为从表/子表，子表中添加的这个数据的DEPARTMENT_ID必须是主表中存在的。主表中这个就叫主键，子表中就叫外键。
...
（1）从表的外键列，必须引用/参考主表的主键或唯一约束的列，若关联的这个列没有主键或唯一约束，就会报错
...
（7）从表的外键列与主表被参照的列名字可以不相同，但是数据类型必须一样，逻辑意义一致。如果类型不一样，创建子表时，就会出现错误“ERROR 1005 (HY000): Can't create table'database.tablename'(errno: 150)”。
	例如：都是表示部门编号，都是int类型。一个叫department_id，一个叫dp_id
...
create table emp( #从表
    ...
    ename varchar(5),     #员工姓名
    deptid int,			#员工所在的部门
    CONSTRAINT fk_empl_dept_id foreign key (deptid) references dept(did)   #在从表中指定外键约束
    #emp表的deptid和和dept表的did的数据类型一致，意义都是表示部门的编号
);
...
# 或者创建一个没有外键约束的约束，然后再ALTER加上外键约束
CREATE TABLE emp1(
emp_id INT PRIMARY KEY AUTO_INCREMENT,
emp_name VARCHAR(15),
department_id INT,
...
ALTER TABLE emp1
ADD CONSTRAINT emp_dept_id_fk FOREIGN KEY(department_id) REFERENCES dept1(dept_id);
...
insert into emp values(2,'李四',1005); # 添加从表记录失败，因为主表没有1005这个部门
...
常规情况下，主表中被子表关联的记录无法update/delete。
...
```mysql
(1) 第一步先查看从表的约束名 和 删除外键约束
SELECT * FROM information_schema.table_constraints
WHERE table_name = '表名称';#查看某个表的约束名，看CONSTRAINT_TYPE为FOREIGN KEY那行的CONSTRAINT_NAME，比如这里就是“外键约束名”
...
(2) 第二步查看索引名和删除索引。（注意，只能手动删除）
show index from emp;
alter table emp drop index emp_ibfk_1;
```
...
给某个字段/某列指定**默认值**，一旦设置默认值，在插入数据时，如果此字段没有显式赋值，则赋值为默认值。
...
```mysql
insert into employee(eid,ename) values(2,'天琪'); # 没有指定，使用默认值
```
...
# 注意，如果tel有唯一性约束的话会报错，因为这就俩空字符串了；如果tel没有唯一性约束，可以添加成功
```
