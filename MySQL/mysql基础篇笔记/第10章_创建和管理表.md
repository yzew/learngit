...
- 保持字段名和类型的一致性：在命名字段并为其指定数据类型的时候一定要保证一致性，假如数据类型在一个表里是整数，那在另一个表里可就别变成字符型了。比如department_id字段，在那几个表中就都是数字
...
```mysql
CREATE DATABASE 数据库名;  # 使用默认的字符集
```

- 方式2：创建数据库并指定字符集

```mysql
CREATE DATABASE 数据库名 CHARACTER SET 字符集;  # 显示的指明字符集
...
CREATE DATABASE IF NOT EXISTS dbtest CHARACTER SET 'gbk';
```
...
### 3.2 创建方式2：基于现有的表，同时导入数据

- 使用 AS subquery 选项，**将创建表和插入数据结合起来**。即将查询语句得到的结果创建出一个表
...
```mysql
# 创建一个表，将employees的所有数据拷贝过来
CREATE TABLE emp1
AS
SELECT * FROM employees;  # 

# 创建一个表，实现对employees表的复制，不包括表数据
CREATE TABLE emp2
AS
SELECT * 
FROM employees 
WHERE 1 = 2;  # 创建的emp2是空表，因为过滤条件是不存在对应值的，比如写department_id > 1000000也行
```
...
FROM    employees
WHERE   department_id = 80;  # 将employees表中部门号为80的拷贝过去
```
...
```mysql
ALTER TABLE dept80 
ADD job_id varchar(15);  # 默认添加到表中的最后一个字段

ALTER TABLE dept80 
ADD job_id varchar(15) FIRST;  # 默认添加到最开头

ALTER TABLE dept80 
ADD job_id varchar(15) AFTER salary; # 添加到salary字段的后面
```
...
```mysql
ALTER TABLE	dept80
MODIFY last_name VARCHAR(30);  # 从15改到25
```
...
```mysql
ALTER TABLE  dept80
DROP COLUMN  job_id;  # COLUMN的意思是列
```
...
从结果可以看出，虽然删除操作时报错了，但是**仍然删除了数据表book1。**因为这分为了两步，先删book1再删book2
...
从结果可以看出，**数据表book1并没有被删除。**即先drop book1，成功了，再drop book2，此时失败了，就会进行回滚，即book1的删除操作也没有成功。即将事务看成了一个整体，要不都做了，要不什么都不做
