...
查询结果是 1.0999999999999999。看到了吗？虽然误差很小，但确实有误差。你也可以尝试把数据类型改成 FLOAT，然后运行求和查询，得到的是1.0999999940395355。显然，误差更大了。
...
- 也可以用DEC,NUMERIC这两个名字代替DECIMAL

  | 数据类型                 | 字节数  | 含义                                                         |
  | ------------------------ | ------- | ------------------------------------------------------------ |
  | DECIMAL(M,D),DEC,NUMERIC | M+2字节 | 有效范围由M和D决定，表示整数位有3位，小数位有2位，小数更多的话会四舍五入。若超出最大范围则会报错 |

  使用 DECIMAL(M,D) 的方式表示高精度小数。其中，M被称为精度，D被称为标度。0<=M<=65，0<=D<=30，D<M。例如，定义DECIMAL（5,2）的类型，表示该列取值范围是-999.99~999.99，若保存999.996，则会报错。
...
> 为什么时间类型 TIME 的取值范围不是 -23:59:59～23:59:59 呢？原因是 MySQL 设计的 TIME 类型，不光表示一天之内的时间，而且可以用来表示一个时间间隔，这个**时间间隔**可以超过 24 小时。
...
> 底层存储的是毫秒数。因此排序的话这个类型会比DATETIME要快
