...
SELECT 100 + '1' # 输出101
SELECT 100 + 'a' # 输出100，将无法转换成整型的当作0
SELECT 100 + null  # 输出null
```
...
> - 在Java中，+的左右两边如果有字符串，那么表示字符串的拼接，如100 + '1'是1001。但是在MySQL中+只表示数值相加。如果遇到非数值类型，先尝试转成数值，如果转失败，就按0计算。（补充：MySQL中字符串拼接要使用字符串函数CONCAT()实现）
>
...
SELECT 12 MOD -5, -12 % -5 # 分别输出2和-2，结果的符号和被模数即第一个数字是一样的
```

```mysql
#筛选出employee_id是偶数的员工
SELECT * FROM employees
WHERE employee_id MOD 2 = 0;  # where要紧跟着from
```
...
# commission_pct中存在有Null的数据，但这么写无法输出含有null的元素，因为只要有null参与，就是null，就被过滤掉了。只有结果为1的才会被呈现
SELECT salary
FROM employees
...
# 这样就可以输出 commission_pct 为 NULL的结果了
...
**3．不等于运算符**
不等于运算符（<>或!=）用于判断两边的数字、字符串或者表达式的值是否不相等，如果不相等则返回1，相等则返回0。不等于运算符不能判断NULL值。如果两边的值有任意一个为NULL，或两边都为NULL，则结果为NULL。
SQL语句示例如下：
...
SELECT employee_id,commission_pct FROM employees WHERE commission_pct = NULL;  # 就这种没法输出为null的
```
...
SELECT LEAST('aae', 'aabc') # 输出aabc，逐元素比较，与长度无关
SELECT LEAST(LENGTH('aae'), LENGTH('aabc')) #输出3，这样就是比较长度了
...
SELECT last_name, salary
FROM   employees
WHERE  salary >= 2500 && salary <= 3500;

# 输出不在[2500, 3500]区间内的值
SELECT last_name, salary
FROM   employees
...
SELECT	first_name
FROM 	employees
WHERE	first_name LIKE 'S%';  # 查找S开头的

SELECT	first_name
FROM 	employees
WHERE	first_name LIKE '%S%';  # 查找含有s的

SELECT	first_name
FROM 	employees
WHERE	first_name LIKE '%S%' AND last_name LIKE '%e%'  # 包含s和e的

SELECT	first_name
FROM 	employees
WHERE	first_name LIKE '%S%e%'  # 包含s和e的，且s在e的前面

SELECT	first_name
FROM 	employees
WHERE	first_name LIKE '%S%e%' AND last_name LIKE '%e%s%'  # 包含s和e的
```

```mysql
SELECT last_name
FROM   employees
WHERE  last_name LIKE '_o%';  # 查询第二个字符是o的

SELECT last_name
FROM   employees
WHERE  last_name LIKE '__o%';  # 查询第三个字符是o的
```
...
SELECT last_name
FROM   employees
WHERE  last_name LIKE '_\_o%';  # 查询第二个字符是_，第三个字符是o的
...
- 可以通过ESCAPE，用其他字符表示转义字符


```mysql
SELECT job_id
FROM   jobs
WHERE  job_id LIKE ‘IT$_%‘ escape ‘$‘;  # 用 $ 表示转义字符
```
...
```mysql
SELECT 'shkstart' REGEXP '^s', 'shkstart' REGEXP 't$', 'shkstart' REGEXP 'hk';  # shkstart是不是以s开头，shkstart是不是以t结尾，shkstart中是不是有hk这个字符
+--------------------+--------------------+---------------------+
