...
```mysql
CREATE [OR REPLACE] # [OR REPLACE]是修改视图的一些行为
[ALGORITHM = {UNDEFINED | MERGE | TEMPTABLE}] # 一些算法
VIEW 视图名称 [(字段列表)]
AS 查询语句
...
# 确定视图中字段名的方式2：
CREATE VIEW vu_emp3(emp_id,NAME,monthly_sal) # 小括号内字段个数与SELECT中字段个数相同，一一对应
AS
SELECT employee_id,last_name,salary 
FROM emps
WHERE salary > 8000;
...
我们经常需要输出某个格式的内容，比如我们想输出员工姓名和对应的部门名，对应格式为 emp_name(department_name)，就是 王皓（华为） 这种形式，就可以使用视图来完成数据格式化的操作：
...
要使视图可更新，视图中的行和底层基本表中的行之间必须存在`一对一`的关系。另外当视图定义出现如下情况时，视图**不支持更新操作**：

- 在定义视图的时候指定了“ALGORITHM = TEMPTABLE”，视图将不支持INSERT和DELETE操作；
- 视图中不包含基表中所有被定义为非空又未指定默认值的列，视图将不支持INSERT操作；
- 在定义视图的SELECT语句中使用了`JOIN联合查询`，视图将不支持INSERT和DELETE操作；
- 在定义视图的SELECT语句后的字段列表中使用了`数学表达式`或`子查询`，创建了新的字段值，对于这个字段值，视图将不支持INSERT，也不支持UPDATE。但支持DELETE
...
视图跟实际的数据表不一样，它存储的是查询语句。所以，在使用的时候，我们要通过定义视图的查询语句来获取结果集。而视图本身不存储数据，不占用数据存储的资源，减少了数据冗余。
