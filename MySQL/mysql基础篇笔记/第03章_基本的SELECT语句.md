...
![image-20230302200823042](D:\MarkDown\picture\image-20230302200823042.png)
...
- 每条命令以 ; 或 \g 或 \G 结束。如果仅执行一条语句，分号的有无是没有影响的，但若选中多条语句执行，则会报错。在一些特殊情况下会使用/g
- 关键字不能被缩写也不能分行。SQLyog中蓝色的就是关键字
- 关于标点符号
...
- **MySQL 在 Windows 环境下是大小写不敏感的**，因为windows本身是不区分大小写的
...
方式1：在命令行客户端登录mysql，使用source指令导入（注意，这条语句必须用命令行执行才可以

方式2：若是想使用SQLyog导入，需要在工具->执行SQL脚本->选择mysqldb.sql文件
...
SELECT 1; #没有任何子句
SELECT 9/2; #没有任何子句
SELECT 1 + 1, 3 * 2; # 查询，显示了1+1和3*2的结果
```
...
FROM     标识从哪个表中选择（表名）
SELECT 1 + 1, 3 * 2
FROM DUAL; # 这里 DUAL 是伪表，执行结果和SELECT 1 + 1, 3 * 2;相同
```
...
- 方式1：**在列名和别名之间加入关键字AS**，其中AS可以省略，用空格隔开即可；
  方式2：**别名使用双引号**，以便在别名中包含空格或特殊的字符并区分大小写。

- 方式2可以实现别名中带有空格，如"王 皓"，这是方式1无法实现的
...
- 举例
  ...
  ```mysql
  # 为last_name起别名为name，为commission_pct起别名为comm
...
- 所有运算符或列值遇到null值，运算的结果都为null。比如下面的例子中，对于 commission_pct 是null的，最后结果就是null，注意，null不等于0，如果将 commission_pct 的null视为0的话，计算出来的结果就不是null了
...
# 正确做法
# 使用IFNULL关键字，遇到null就替换为0：IFNULL(commission_pct,0)，在单行函数那节会讲
SELECT employee_id,salary,commission_pct,
12 * salary * (1 + IFNULL(commission_pct,0)) "annual_sal"
FROM employees;
```

这里你一定要注意，在 MySQL 里面， 空值不等于0，也不等于空字符串。一个空字符串的长度是 0，而一个空值的长度是空。而且，在 MySQL 里面，空值是占用空间的。

### 3.5 着重号

- 错误的

因为order这个表名是个关键字，如果表名和保留字、常用方法等冲突，需要使用着重号括起来，才会被识别为表名
...
![image-20230302211457528](D:\MarkDown\picture\image-20230302211457528.png)

## 4. 显示表结构

使用DESCRIBE 或 DESC 命令，表示表结构。显示的是表中字段的详细信息，比如每个列的名字、数据类型、可不可以为Null等等
...
SELECT last_name
FROM employees
WHERE last_name = 'King'; # 字符串用单引号表示
```

![1554952277028](images/1554952277028.png)

![image-20230302212013603](D:\MarkDown\picture\image-20230302212013603.png)
