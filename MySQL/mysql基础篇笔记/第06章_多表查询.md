...
* 冗余。即比如一些人都是一个部门，那在原来表的基础上都会增加departments表中的四个字段，这些重复部门的这四个字段都是重复的。如果有的人没有部门，就会在后面增加四个null字段，这些都是冗余的。这就会导致I/O传输时的效率比较低
* 笛卡尔积 
...
# 可以发现，结果就是部门数 × 员工数，即每个员工和每个部门都匹配了一下。这就是笛卡尔积错误
SELECT 107*27 FROM dual;
```

![1554975097631](D:\MarkDown\picture\1554975097631.png)
...
![image-20230304194544499](D:\MarkDown\picture\image-20230304194544499.png)
...
- 加入连接条件后，查询语法：
  ...
  # 案例：查询员工的姓名及其部门名称
  # 输出106行数据，为什么不是107呢，因为有个人的部门是null
  SELECT last_name, department_name
  FROM employees, departments
...
- 在不同表中具有相同列名的列可以用`表名`加以区分。
  - 从sql优化的角度，建议多表查询时，**每个字段前都指明**其所在的表。查询速度会更快

```mysql
# 当我们要显示department_id时，报错了
# 因为两个表中都有这个字段，我们要是要显示的话，必须指明找哪个表里的
SELECT employee_id,department_name,department_id
FROM employees,departments
WHERE employees.department_id = departments.department_id;

# 正确写法：employees.department_id
...
> 需要注意的是，如果我们使用了表的别名，在查询字段中、过滤条件中就**只能使用别名**进行代替，不能使用原有的表名，否则就会报错。
...
​	连接 n个表，至少需要n-1个连接条件。比如，连接三个表，至少需要两个连接条件，各个条件可以用AND连接
...
题目：查询员工id,员工姓名及其管理者的id和姓名。这些元素同属一个表格
...
# 即只是把左表和右表中满足连接条件的行返回了，其他结果都没有要
SELECT employee_id,department_name
FROM employees e,departments d
WHERE e.`department_id` = d.department_id;  
# 只有106条记录，那个没有部门的没有显示
```
...
- 内连接: 合并具有同一列的两个以上的表的行, **结果集中不包含一个表与另一个表不匹配的行**，即只是把左表和右表中满足连接条件的行返回了，其他结果都没有要

- 外连接: 两个表在连接过程中除了返回满足连接条件的行以外**还返回左（或右）表中不满足条件的行**，这种连接称为**左（或右）外连接**。没有匹配的行时, 结果表中相应的列为空(NULL)。

- 外连接分为左外连接、右外连接和满外连接

- 左外连接补充上了左表中不满足的情况。如果是左外连接，则连接条件中左边的表也称为`主表`，右边的表称为`从表`。

  右外连接补充上了右表中不满足的情况。如果是右外连接，则连接条件中右边的表也称为`主表`，左边的表称为`从表`。
  满外连接则是两个表不满足的情况都展示出来
...
- Oracle 对 SQL92 支持较好，而 MySQL 则不支持 SQL92 的外连接。
  ...
  SELECT employee_id,department_name
  FROM employees e,departments d
  WHERE e.`department_id` = d.department_id(+);  # 将e作为主表
...
```mysql
SELECT 字段列表
FROM A表 INNER JOIN B表 # 可以使用 JOIN 或 INNER JOIN
ON 关联条件
WHERE 等其他子句;
```
...
> 一般题目中让查 **所有的** 员工的last_name,department_name信息，这就是让你实现**外连接**
...
#实现查询结果是A
SELECT 字段列表
FROM A表 LEFT OUTER JOIN B表 # OUTER 可省略
ON 关联条件
WHERE 等其他子句;
```
...
UNION ALL  # 这两个集合不存在交集，因此用UNION ALL就行，效率会更高
