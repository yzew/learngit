...
```mysql
# 对于有参数的，如这两个 RAND(10) ，得到的结果是一样的
# 放的这个随机数因子是一样的，得到的结果就会是一样的，并且这个随机数就不会在变化了
...
![image-20230304223342203](D:\MarkDown\picture\image-20230304223342203.png)
...
| ACOS(x)    | 返回x的反余弦值，即获取余弦为x的值。如果x的值不在-1到1之间，则返回NULL。返回值是弧度值 |
...
```mysql
# 如这里要计算sin(30)，要先把角度转换为弧度，再用sin进行计算
...
```mysql
# 65 5 2 5 6，最后的6是因为汉字在UTF8中占3个字节
...
| **NOW()** / SYSDATE() / CURRENT_TIMESTAMP() / LOCALTIME() / LOCALTIMESTAMP() | 返回当前系统日期和时间，记住其中一个就行         |
| UTC_DATE()                                                   | 返回UTC（世界标准时间）日期                      |
| UTC_TIME()                                                   | 返回UTC（世界标准时间）时间，比中国时间少8个小时 |
...
| 函数                    | 用法                                                         |
| ----------------------- | ------------------------------------------------------------ |
| EXTRACT(type FROM date) | 返回指定日期中特定的部分，type指定返回的值，如SELECT EXTRACT(MINUTE FROM NOW()) |
...
# 如果 commission_pct 是null，输出0; 不是null，输出commission_pct自己
...
| 函数                        | 用法                                                         |
| --------------------------- | ------------------------------------------------------------ |
| PASSWORD(str)               | 返回字符串str的加密版本，41位长的字符串。加密结果`不可逆`，常用于用户的密码加密。8.0中被弃用 |
| MD5(str)                    | 返回字符串str的md5加密后的值，也是一种加密方式，也是不可逆的。若参数为NULL，则会返回NULL |
| SHA(str)                    | 从原明文密码str计算并返回加密后的密码字符串，当参数为NULL时，返回NULL。`SHA加密算法比MD5更加安全`。 |
| ENCODE(value,password_seed) | 返回使用password_seed作为加密密码加密value，在8.0中被弃用    |
| DECODE(value,password_seed) | 返回使用password_seed作为加密密码解密value，在8.0中被弃用    |
...
# 这里对年数进行了四舍五入
SELECT employee_id, hire_date, ROUND(DATEDIFF(CURDATE(),hire_date) / 365) "years", DATEDIFF(CURDATE(),hire_date) "days"
FROM employees
ORDER BY years DESC;
